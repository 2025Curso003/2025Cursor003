    ...
    while True:
        try:
            ...
            browser_email_manager = BrowserManagerEmail()
                ...
                # 获取邮箱验证码
            if email_password.endswith("@apiget.com"):
                # 获取邮箱验证码
                try:
                    ...
                    response = requests.get(f"https://00mail.cn/prod-api/email/api/stand/getNewEmailContent?email={email}")
                    email_content = response.text
                    
                    # 使用正则表达式查找验证码
                    code_match = re.search(r"Enter the code below in your open browser window. (\d[\s\d]*\d)", email_content)
                    if code_match:
                        # 提取数字并移除空格
                        code = ''.join(code_match.group(1).split())
                        if len(code) == 6:
            ...
            elif email.endswith("@7to.us") or email.endswith("@gmail.com"):
