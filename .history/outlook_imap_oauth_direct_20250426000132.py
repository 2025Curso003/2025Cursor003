...
def get_verify_code(username, client_id, refresh_token):
    """获取邮件中的验证码
    Args:
...
def get_verification_code(username, client_id, refresh_token, password=None, max_retries=3, retry_interval=5):
    """
    获取Cursor的邮箱验证码
    ...
    Args:
        ...
        password (str, optional): 邮箱密码（如果OAuth认证失败，会尝试使用密码）
        max_retries (int, optional): 最大重试次数
        retry_interval (int, optional): 重试间隔（秒）
        
    Returns:
        str or None: 成功返回验证码，失败返回None
    """
    print(f"正在尝试获取 {username} 的Cursor验证码...")
    ...
    for attempt in range(max_retries):
        ...
        if access_token:
            email_data = get_email_with_oauth(username, access_token, mark_as_read=True)
            ...
            if email_data and "body" in email_data:
                ...
                code_match = re.search(r"\b\d{6}\b", email_data["body"])
                if code_match:
                    print(f"成功获取验证码: {code_match.group()}")
                    return code_match.group()
                else:
                    print("未能从邮件中提取到验证码")
        
        # 如果OAuth方式失败且提供了密码，尝试密码方式
        if password:
            email_data = get_email_with_password(username, password, mark_as_read=True)
    ...
    print(f"在 {max_retries} 次尝试后未能获取验证码")
    return None
