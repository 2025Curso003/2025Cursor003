    ...
    while True:
        try:
            ...
            if tab.ele("@data-index=0"):
                ...
                code = email_handler.get_verification_code_outlook(browserEmail, tabEmail, account, password)
                if not code:
...
def save_token_and_email(cursor_token, cursor_email, cursor_password, cursor_name, expires_time, cursor_user_sub=None):
    connection = None
    try:
        # 连接到MySQL数据库
        ...
        with connection.cursor() as cursor:
            # 插入数据的SQL语句
            insert_query = """
            INSERT INTO cursor_token_info (cursor_token, cursor_email, cursor_password, cursor_name, expires_time, cursor_user_sub)
            VALUES (%s, %s, %s, %s, %s, %s)
            """
            # 执行插入操作
            cursor.execute(insert_query, (cursor_token, cursor_email, cursor_password, cursor_name, expires_time, cursor_user_sub))
            # 提交事务
            connection.commit()
    ...
    try:
        ...
        for email, email_password in emails:
            ...
            if sign_up_account(browser, tab, email, email_password):
                ...
                save_token_and_email(token, email, password, f"{first_name} {last_name}", expires_time.strftime('%Y-%m-%d %H:%M:%S'), None)
