...
class GetVerificationCodeGmail:
    ...
    def get_verification_code_gmail(self, browser, tab, account, password, max_retries=2, retry_interval=10):
        """
        获取验证码，带有重试机制。
    ...
    def _get_mail_code_by_imap(self, browser, tab, account, password, retry=2):
        ...
        try:
            ...
            # 使用隐私模式打开新标签页
             ...
             logging.info("提交邮箱信息...")
             next_button = tab.ele("font:contains('下一步')")
             if next_button:
             ...
             logging.info("提交密码信息...")
             next_button = tab.ele("font:contains('下一个')")
             if next_button:
                 logging.info("找到 'next' 按钮，正在点击...")
                 next_button.click()
             else:
                 logging.error("未找到 'next' 按钮")
             time.sleep(3)
             logging.info("开始跳过我了解...")
             try:
                logging.info("正在点击我了解...")
                email_input = tab.ele("@type=submit")
                if email_input:
                    email_input.click()
                else:
                    logging.error("未找到我了解")
             except Exception as e:
                logging.error(f"未找到我了解: {e}")

             time.sleep(3)
             logging.info("开始跳过Do this later...")
             try:
                logging.info("正在点击Do this later...")
                email_input = tab.ele("@class=Z6C2jc")
                if email_input:
                    email_input.click()
                else:
                    logging.error("未找到Do this later")
             except Exception as e:
                logging.error(f"未找到Do this later: {e}")
             
             logging.info("保护跳过完成...")
             time.sleep(20)
             logging.info("睡眠20秒完成...")
             # 循环获取三次
             for i in range(6):
