...
class GetVerificationCode163:
    def __init__(self):
        self.login_url_163 = "https://mail.163.com";

    def get_verification_code_163(self, browser, tab, email, email_password, max_retries=2, retry_interval=10):
        """
        获取验证码，带有重试机制。
        ...
        for attempt in range(max_retries):
            try:
                ...
                verify_code = self._get_mail_code_by_imap(browser, tab, email, email_password)
    ...
    # 使用imap获取邮件
    def _get_mail_code_by_imap(self, browser, tab, email, email_password, retry=2):
        if retry > 0:
            time.sleep(3)
        if retry >= 3:
        ...
        try:
            ...
            # 使用隐私模式打开新标签页
             logging.info(f"正在访问邮箱登录页面: {self.login_url_163}")
             # 打开新标签进行请求
             tab.get(self.login_url_163)
             time.sleep(2)  # 等待页面加载

             try:
                logging.info("正在填写邮箱...")
                email_input = tab.ele("@name=email")
                if email_input:
                    # account 去掉邮箱后缀
                    email = email.split("@")[0]
                    email_input.click().input(email)
                else:
                    logging.error("未找到邮箱输入框")
                logging.info(f"已输入邮箱......")
             except Exception as e:
             ...
             try:
                ...
                if password_input:
                    password_input.click().input(email_password)
                else:
             ...
             #点击收件箱
             for i in range(3):
                logging.info("开始查找收件箱...")
                # 对id为 animMailTongueDown 进行点击
                anim_mail_tongue_down = tab.ele("#animMailTongueDown")
                if anim_mail_tongue_down:
                    anim_mail_tongue_down.click()
                    logging.info("点击收件箱完成...")
                    break
                else:
                    logging.info("没有找到收件箱，睡眠2秒继续获取...")
                    time.sleep(2)
             time.sleep(5)

             elements = tab.eles('css:div.gB0[sign="start"]')
             if elements and len(elements) > 0:
                ...
                time.sleep(0.5)  # 短暂延迟
                # 执行点击
                first_element.click(timeout=3)  # 设置超时时间
                logging.info("成功点击第一个元素")
             else:
                logging.error("未找到目标元素")

             time.sleep(15)
             logging.info("开始查找验证码div...")
             ...
             try:
                 ...
                 logging.error(f"等待页面加载时出错: {e}")
             
             # 使用更简单的选择器先尝试
             try:
                 # 先尝试查找所有div
                 all_divs = tab.eles('css:div')
                 logging.info(f"页面中div总数: {len(all_divs)}")
                 
                 # 然后使用原始选择器
                 verification_divs = tab.eles("div:style-matches('font-family:-apple-system, BlinkMacSystemFont, 'Segoe UI', Helvetica, Arial, sans-serif;font-size:28px;font-weight:400;letter-spacing:2px;line-height:30px;text-align:left;color:#202020;')")
                 logging.info(f"使用样式选择器找到的元素数量: {len(verification_divs)}")
                 
                 if not verification_divs:
                     logging.warning("未找到匹配样式的div，尝试其他方法...")
                     # 尝试查找所有包含数字的div
                     all_text_divs = tab.eles('css:div')
                     for div in all_text_divs:
                         if div.text and re.match(r'^\d{6}$', div.text.strip()):
                             verification_divs = [div]
                             logging.info("通过文本内容找到验证码div")
                             break
             except Exception as e:
                 logging.error(f"查找验证码div时出错: {e}")
                 verification_divs = []
             
             verification_code_div = None
             for div in verification_divs:
                 if div.text and re.match(r'^\d{6}$', div.text.strip()):
                     verification_code_div = div
                     break
                 
             logging.info(f"找到的元素: {verification_code_div}")
             
             if verification_code_div:
                logging.info("找到6个数的元素文本...")
                # 点击6个数的元素文本
                verification_code = verification_code_div.text.strip()
                logging.info(f"得到验证码: {verification_code}")
                time.sleep(2)
